...
The beta is published under npm's `next` channel rather than replacing the stable `latest` release. The current source tree requires Node.js 22.19 or newer and the exact `@earendil-works/pi-*` 0.82.1 packages used by Pi 0.82.1.

> **Pi version compatibility:** This source build targets Pi 0.82.1 exactly; earlier and later Pi SDK versions are not claimed as compatible. The published `0.4.0-beta.1` package originally targeted Pi 0.80.10, so use a release that explicitly names Pi 0.82.1 or build the current source tree. After installing or rebuilding pi-forge, restart Pi so the extension and host SDK agree. If the parent agent can use a provider but subagent preparation reports `No API key found`, check for a stale pi-forge build before using `/login`: mismatched session/runtime APIs can lose the parent authentication during preparation, and logging in again does not fix that version mismatch.
...
The same editor manages agent profiles: use the top navigation to switch between prompt stacks and agent profiles. The profile list shows each profile's ID, name, model/thinking/stack targets, validation state, and auto-activation, last-applied, and delegation badges. Trusted projects can create, edit, validate, save, apply once, and delete profiles without writing JSON: model and thinking controls are populated from the model registry, the prompt-stack control from the shared stack repository, and saving rejects a second auto-activation profile. A runtime and provenance card distinguishes the current runtime from last-applied provenance and reports source-definition changes separately from per-field runtime drift after external model, thinking-level, or prompt-stack changes.

Each profile also has a delegation card for the experimental subagent path. It reports the effective delegation state, backend, timeout, and their configuration sources, and trusted projects can toggle delegation and override backend/timeout without hand-editing `.pi/forge/config.json`. Project defaults (`subagents.backend`, `subagents.timeoutMs`) and `subagents.allowAgentInvocationWithoutApproval` stay config-file-only.
