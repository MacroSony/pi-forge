...
The `pi-subprocess-readonly` and `pi-rpc-readonly` paths send one requested text task through profile resolution, backend preflight, exact backend-assisted prompt preparation, immutable plan validation, interactive review by default, a foreground Pi process (text/print or RPC), and normalized response handling without owning a general runner. Backend registration, preflight binding, plan sealing, lifecycle arbitration, and both process backends are owned by `@zihanw/pi-subagent-runtime`; pi-forge keeps profiles, compilation, approval, and presentation. The default backend is layered configuration (global `subagents.backend`, trusted-project override, per-run `--backend`) with no fallback, and a trusted-project option can explicitly authorize the model-callable tool without per-run review pinned to the configured backend. The former in-package `SubagentBackendRegistry`, `PiSubprocessBackend`, and `PiSdkIsolatedBackend` exports were removed in the migration; the SDK spike harness was removed after its findings were productized in the runtime's shared preparation gate.
