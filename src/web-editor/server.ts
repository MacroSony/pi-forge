...
function readStackPayload(body: unknown): { ok: true; stack: PromptStack } | { ok: false; error: string } {
	...
	if (!isPlainObject(rawStack)) return { ok: false, error: "Preset payload must be a JSON object." };

	if (typeof rawStack.id !== "string" || !rawStack.id.trim()) return { ok: false, error: "Preset id must be a non-empty string." };
	if (!Array.isArray(rawStack.items)) return { ok: false, error: "Preset must contain a Stack items array." };
