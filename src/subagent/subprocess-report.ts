...
function sanitizeReportString(value: string): string {
	...
	if (looksLikeBase64Payload(value, bytes)) return `[Base64-like data omitted from retained subagent report: ${bytes} encoded bytes]`;
...
function utf8Prefix(value: string, maxBytes: number): string {
	...
	while (prefix.endsWith("\uFFFD") && end > 0) prefix = buffer.subarray(0, --end).toString("utf8");
	return prefix;
}
