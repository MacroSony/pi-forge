 ...
 */
export function parsePromptStack(source: string, filePath: string, scope: PromptStackScope): LoadedPromptStack {
...
function dedupeDiagnostics(diagnostics: PromptStackDiagnostic[]): PromptStackDiagnostic[] {
	const seen = new Set<string>();
	const result: PromptStackDiagnostic[] = [];
...
export function createPromptStackFault(
	filePath: string,
...
export function validatePromptStack(stack: PromptStack): PromptStackDiagnostic[] {
	...
	if (!stack.id.trim()) diagnostics.push({ level: "error", message: "Preset id must not be empty." });
