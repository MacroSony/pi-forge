...
export function hasRequestFrequencyRules(stack: PromptStack): boolean {
 ...
 */
export function applyRequestFrequencyRulesToMessages(
	stack: PromptStack,
	messages: AgentMessage[],
	diagnostics: PromptStackDiagnostic[],
): AgentMessage[] {
...
function requestFrequencyMessageRules(stack: PromptStack, diagnostics: PromptStackDiagnostic[]): CompiledRegexRule[] {
	const rules = Array.isArray(stack.regex?.rules) ? stack.regex.rules : [];
	const historyRules: CompiledRegexRule[] = [];
	const compiledRules: CompiledRegexRule[] = [];

	for (const rawRule of rules) {
		if (!isPlainObject(rawRule)) continue;
		if (rawRule.enabled === false) continue;
		if (rawRule.frequency !== "request") continue;
		if ((rawRule.effect ?? "outgoing") !== "outgoing") continue;
		if (rawRule.stage === "compiled" && Array.isArray(rawRule.targets) && !rawRule.targets.includes("messages")) continue;
		const rule = compileRuntimeRule(rawRule as unknown as PromptRegexRule, diagnostics);
		if (!rule) continue;
		if (rule.stage === "history") historyRules.push(rule);
		else compiledRules.push(rule);
	}
...
export function applyFinalizeRegexRulesToMessage(
	...
	if (!FINALIZE_ROLES.has(role)) return undefined;
	...
	for (const rule of regexRulesFor(stack, "compiled", "messages", "finalize", diagnostics)) {
		...
		if (role === "toolResult" && !rule.roles?.includes("toolResult")) continue;
		const stats: RegexStats = { matches: 0, changedSegments: 0 };
...
function validateRule(rawRule: unknown, index: number, seenIds: Set<string>, diagnostics: PromptStackDiagnostic[]): void {
	...
	if (rawRule.frequency !== undefined) {
		if (!VALID_FREQUENCIES.has(rawRule.frequency as string)) {
...
function compileRuntimeRule(rule: PromptRegexRule, diagnostics: PromptStackDiagnostic[]): CompiledRegexRule | undefined {
	...
	try {
			return {
				...
				stage: rule.stage,
				effect,
				frequency: VALID_FREQUENCIES.has(rule.frequency as string) ? rule.frequency as PromptRegexFrequency : "turn",
