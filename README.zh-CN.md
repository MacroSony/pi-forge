...
- **转换发出的 prompt 文本** — 对选中的历史或最终编译 prompt 执行确定性 regex 替换。
...
Prompt stack 可以对发给模型的 prompt 文本执行确定性的 regex 替换。当前第一版只支持 `history` 和 `compiled` 阶段；display-only 和 provider-payload 重写还不会生效。
...
当前 MVP 只有 `effect: "outgoing"` 会改变发给模型的输入。`effect: "display"` 和 `"both"` 会通过校验并产生 warning，但在 display transforms 实现前运行时会忽略。
