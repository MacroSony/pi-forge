...
- **按栈限制工具和技能** — 为专注模式启用工具 allow/deny 策略，并过滤技能可见性。
...
拖拽、编辑、校验、查看完整预览和捕获的 payload、用 tabs 管理变量/状态/context/regex 规则、切换深色模式、通过原始 stack JSON 修复高级字段、导入、导出、fork、删除栈 —— 全在浏览器里完成。Stack metadata 可以折叠，方便把当前编辑区留在屏幕内。
...
可安全表示的 SillyTavern `promptOnly` regex 脚本会转换成 pi-forge `regex.rules`。Display-only、prompt/display 混合、DOM/browser、CSS/HTML 美化、JavaScript 和不支持的 regex 脚本会保留为报告项，供手动检查。
...
Prompt stack 可以用栈级 `allow` 和 `deny` 列表限制工具和技能。模式默认精确匹配，也支持 `*` 通配符。
...
具体的 `allow` 模式优先，并定义最终允许集合；即使同名也匹配 `deny`，仍会保留。只有当 `allow` 缺失或只包含 `"*"` 时才应用 `deny`，因此 `allow: ["*"]` 搭配 `deny` 可作为纯 deny 策略使用。
...
Web 编辑器提供结构化 Regex 对话框来编辑这些规则字段，并会保留需要通过 raw JSON 编辑的高级未知字段。
