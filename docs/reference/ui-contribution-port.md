...
Status: experimental, versioned (`UI_CONTRIBUTION_PORT_VERSION = 1`). The `@zihanw/pi-forge/ui-contribution` entry point is the generic cross-extension port that lets optional packages contribute schema-driven tabs to the pi-forge web editor. The forge side knows nothing about specific providers; the first consumer is [`@zihanw/pi-forge-subagents`](https://github.com/MacroSony/pi-forge-subagents), which contributes its Subagent Settings tab when installed.
...
- **pi-forge owns** provider discovery over the bus, rendering contributed tabs through the generic schema-form renderer, and proxying browser writes back over the bus through the web server routes. It never interprets or stores contributed configuration itself.
...
4. Provider disposal sends `unavailable`. The web editor clears that provider's contributed tabs and re-discovers when a provider reappears across sessions; late-surfacing providers are picked up without a page reload path change.
