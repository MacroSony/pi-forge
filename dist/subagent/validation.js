import { FINGERPRINT_PATTERN, LIMIT_NAMES, OPAQUE_ID_PATTERN, error, hasErrors, isFingerprint, isRecord, isSafeRelativePath, validateAccessEnforcement, validateAccessRequest as validatePortableAccessRequest, validateBackendDescriptor as validatePortableBackendDescriptor, validateLimitReceipt as validatePortableLimitReceipt, validateLimitRequest as validatePortableLimitRequest, validatePromptRuntime as validatePortablePromptRuntime, } from "@zihanw/pi-subagent-runtime";
// ---------------------------------------------------------------------------
...
export function validateBackendDescriptor(value, path, diagnostics) {
    ...
    if (isRecord(value.capabilities)) {
        for (const field of ["traceInspection", "artifactRetention"]) {
            if (typeof value.capabilities[field] !== "boolean")
    ...
    if (!isRecord(value.enforcement))
        ...
        if (executionBoundary === "shared-user") {
            for (const field of ["readOnlyMountIsolation", "readWriteMountIsolation", "symlinkSafeContainment", "processIsolation", "agentNetworkIsolation"]) {
                if (value.enforcement[field] === true)
                    diagnostics.push(error("access-receipt.shared-user-claim", `A shared-user boundary cannot claim ${field}.`, `${path}.enforcement.${field}`));
            }
        }
        if (executionBoundary === "isolated" && value.level === "read-only" && (!value.enforcement.readOnlyMountIsolation || !value.enforcement.symlinkSafeContainment))
            diagnostics.push(error("access-receipt.read-isolation", "Isolated read-only access requires mount isolation and symlink-safe containment.", `${path}.enforcement`));
...
function repath(diagnostics, path) {
    return diagnostics.map((diagnostic) => {
        if (!diagnostic.path || !diagnostic.path.startsWith("$"))
            return diagnostic;
