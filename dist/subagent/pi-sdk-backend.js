...
export const PI_SDK_ISOLATED_BACKEND_DESCRIPTOR = {
...
export class PiSdkIsolatedBackend {
    ...
    preflight(input) {
        ...
        if (input.request.access.network !== "deny")
            diagnostics.push(errorDiagnostic("pi-sdk.network", "The isolated Pi SDK backend requires agent network policy deny.", "access.network"));
        if ((input.request.input.media?.length ?? 0) > 0)
            diagnostics.push(errorDiagnostic("pi-sdk.media", "The first isolated Pi SDK backend iteration supports text tasks only.", "input.media"));
        for (const name of ["maxTurns", "tokenBudget", "maxOutputBytes"]) {
            ...
            if (requirement?.enforcement === "required")
                diagnostics.push(errorDiagnostic("pi-sdk.limit", `${name} cannot be enforced by the isolated Pi SDK backend.`, `limits.${name}`));
            else if (requirement)
                diagnostics.push(warningDiagnostic("pi-sdk.limit-ignored", `${name} is unsupported and will not be accepted.`, `limits.${name}`));
        }
        const model = this.#modelRegistry.find(input.snapshot.profile.model.provider, input.snapshot.profile.model.id);
        if (!model)
            diagnostics.push(errorDiagnostic("pi-sdk.model", `Unknown model: ${input.snapshot.profile.model.provider}/${input.snapshot.profile.model.id}`, "profile.model"));
        else {
            if (!this.#modelRegistry.hasConfiguredAuth(model))
                diagnostics.push(errorDiagnostic("pi-sdk.auth", `Model ${model.provider}/${model.id} has no configured authentication.`, "profile.model"));
            ...
            if (effectiveThinking !== input.snapshot.profile.thinkingLevel)
                diagnostics.push(errorDiagnostic("pi-sdk.thinking", `Model ${model.provider}/${model.id} would clamp thinking level ${input.snapshot.profile.thinkingLevel} to ${effectiveThinking}.`, "profile.thinkingLevel"));
        }
    ...
    async prepare(input, context) {
        if (this.#primed.has(input.preflight.preflightId))
            throw new Error(`Pi SDK preflight is already prepared: ${input.preflight.preflightId}`);
        ...
        try {
            ...
            if (!prepared || canonicalSubagentJson(prepared) !== canonicalSubagentJson(result))
                throw new Error("Pi SDK preparation bridge returned inconsistent host preparation.");
            const primed = {
    ...
    async execute(plan, context) {
        ...
        if (!primed || primed.requestId !== plan.requestId)
            throw new Error("Pi SDK execution has no matching prepared session.");
        ...
        try {
            ...
            if (terminal.status === "cancelled")
                return { ...common, status: "cancelled", reason: terminal.error ?? "SDK session aborted" };
    ...
    #compilerBridge(input, context, providerGate, preparationReady, setPrepared) {
        return (pi) => {
            ...
            pi.on("context", () => {
                if (!prepared)
                    throw new Error("Pi SDK context event arrived before host preparation.");
    ...
    async #startPrompt(session, task, preparationReady) {
        try {
    ...
    #requireModel(provider, id) {
        ...
        if (!model)
            throw new Error(`Pi SDK model disappeared after preflight: ${provider}/${id}`);
        return model;
    }
...
function preparedMessageToAgentMessage(message, preflight, index) {
    if (message.content.some((part) => part.type === "media"))
        throw new Error("Media preparation is not implemented by the first Pi SDK backend iteration.");
    const content = message.content.map((part) => ({ type: "text", text: part.type === "text" ? part.text : "" }));
    const timestamp = index;
    if (message.role === "user")
        return { role: "user", content: content.length === 1 ? content[0].text : content, timestamp };
    if (message.role === "custom")
        return { role: "custom", customType: "pi-forge-subagent", content, display: false, details: {}, timestamp };
    return {
        role: "assistant",
        content,
        api: "unknown",
...
function terminalResult(session) {
    ...
    if (assistant?.stopReason === "aborted")
        return { status: "cancelled", error: assistant.errorMessage ?? "SDK session aborted", output };
    if (assistant?.stopReason === "error" || assistant?.errorMessage)
        return { status: "failed", error: assistant.errorMessage ?? "Provider returned an error.", output };
...
async function abortable(promise, signal) {
    if (!signal)
        return promise;
    if (signal.aborted)
        throw new Error("Pi SDK preparation was cancelled.");
...
//# sourceMappingURL=pi-sdk-backend.js.map