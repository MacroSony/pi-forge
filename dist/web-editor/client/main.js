...
import { attr, el, escapeHtml, query, queryAll } from "./dom.js";
import { createInspector } from "./inspector.js";
import { createPolicyEditor } from "./policy-editor.js";
import { createRegexEditor } from "./regex-editor.js";
...
const { validateStack, previewStack, refreshPayloadCapture, armPayloadCapture, clearPayloadCapture, openPayloadCapture, hidePreview, copyPreviewText, copyTextToClipboard, } = createInspector({
    api,
...
function updateActionState() {
    ...
    if (deleteItemButton)
        deleteItemButton.disabled = !hasStack || selectedItemIndex < 0;
...
async function loadStacks(preferId = selectedId) {
    ...
    if (next)
        await selectStack(next.id, { keepDirty: false });
    else
        renderEmpty();
}
async function selectStack(id, options = {}) {
    if (dirty && !options.keepDirty && !confirm("Discard unsaved changes?"))
        return;
...
function renderSettings() {
    ...
    el("stackDescription").oninput = (event) => { setOptionalString(stack, "description", event.target.value); markDirty(); };
}
...
function updateItemDropIndicator() {
    ...
    list.classList.toggle("drag-active", dragIndex !== -1);
    if (dragIndex === -1 || dragDropIndex === -1 || !rows.length)
        return;
    if (dragDropIndex <= 0)
        rows[0].classList.add("drop-before");
    else if (dragDropIndex >= rows.length)
        rows[rows.length - 1].classList.add("drop-after");
    else
        rows[dragDropIndex].classList.add("drop-before");
}
function updateItemDragAutoScroll(clientY) {
    ...
    let speed = 0;
    if (clientY < rect.top)
        speed = -Math.min(30, 8 + (rect.top - clientY) / 3);
    else if (clientY < rect.top + edge)
        speed = -Math.min(22, (rect.top + edge - clientY) / 3);
    else if (clientY > rect.bottom)
        speed = Math.min(30, 8 + (clientY - rect.bottom) / 3);
    else if (clientY > rect.bottom - edge)
        speed = Math.min(22, (clientY - (rect.bottom - edge)) / 3);
    dragScrollSpeed = speed;
    if (speed !== 0 && !dragScrollFrame)
        dragScrollFrame = requestAnimationFrame(runItemDragAutoScroll);
}
function runItemDragAutoScroll() {
    dragScrollFrame = 0;
    if (dragIndex === -1 || dragScrollSpeed === 0)
        return;
    const list = el("itemList");
...
function diagnosticsForItems() {
    ...
    for (const diagnostic of latestDiagnostics || []) {
        if (!diagnostic.itemId)
            continue;
        if (!grouped[diagnostic.itemId])
            grouped[diagnostic.itemId] = [];
...
function renderItemEditor() {
    ...
    if (item.kind === "slot")
        el("itemSlot").value = item.slot || "chat-history";
    el("itemKind").onchange = (event) => {
        if (event.target.value === item.kind)
            return;
...
function renderSlotOptionsForm(item, options) {
    ...
    if (item.slot === "chat-history") {
        fields.push(optionCheckbox("includeLastUserMessage", "Include last user message", options.includeLastUserMessage !== false), optionCheckbox("stripAssistantThinking", "Strip assistant thinking", options.stripAssistantThinking === true), optionCheckbox("includeSummaries", "Include summaries", options.includeSummaries !== false), optionSelect("toolMode", "Tool history", options.toolMode || "keep", ["keep", "drop"]), optionText("roles", "Roles", Array.isArray(options.roles) ? options.roles.join(", ") : ""), optionNumber("maxMessages", "Max messages", options.maxMessages ?? ""), optionNumber("maxChars", "Max chars", options.maxChars ?? ""));
    }
    if (item.slot === "variables") {
        fields.push(optionCheckbox("includeStatic", "Include static variables", options.includeStatic !== false), optionCheckbox("includeSession", "Include session variables", options.includeSession !== false), optionCheckbox("includeTurn", "Include turn variables", options.includeTurn !== false), optionSelect("format", "Format", options.format || "xml", ["xml", "plain"]));
    }
...
function setSlotOption(item, key, value, defaultValue = undefined) {
    const options = { ...(item.options || {}) };
...
function defaultSlotOptionValue(key) {
    if (["includeLastUserMessage", "includeSummaries", "includeStatic", "includeSession", "includeTurn"].includes(key))
        return true;
    if (["stripAssistantThinking", "includeTime"].includes(key))
        return false;
    if (key === "toolMode")
        return "keep";
    if (key === "format")
...
function openContextEditor() {
    if (!currentStack)
        return;
    showStackModal("Context options", "Stack-level behavior for how pi-forge rewrites Pi conversation context.", '<div class="modal-toolbar"><span class="modal-meta">Save writes these changes to the stack JSON.</span></div>' +
...
function openRawStackJsonEditor() {
    if (!currentStack)
        return;
    const json = JSON.stringify(stackForDisplay(), null, 2);
    showStackModal("Stack JSON", "Raw recovery view for advanced fields. Apply updates the editor; Save writes to disk.", '<div class="modal-toolbar">' +
        '<button id="copyStackJsonBtn" data-icon="□" title="Copy this JSON to the clipboard">Copy</button>' +
        '<button id="applyStackJsonBtn" class="primary" data-icon="✓" title="Apply this JSON to the editor without saving">Apply to editor</button>' +
        '<span class="modal-spacer"></span><span id="stackJsonStatus" class="modal-meta">Unsaved stack JSON draft.</span>' +
        '</div>' +
        '<textarea id="stackJsonText" class="raw-json-editor" spellcheck="false">' + escapeHtml(json) + '</textarea>', { bodyClass: "json-modal" });
    el("copyStackJsonBtn").onclick = () => run(copyRawStackJson);
    el("applyStackJsonBtn").onclick = () => run(applyRawStackJson);
}
function stackForDisplay() {
    ...
    if (!clone.type)
        clone.type = "pi-forge.prompt-stack";
...
async function applyRawStackJson() {
    ...
    const stack = parsed;
    currentStack = stack;
    if (!stack.schemaVersion)
        stack.schemaVersion = 1;
    if (!stack.type)
        stack.type = "pi-forge.prompt-stack";
...
function openVariablesEditor() {
    ...
    const rows = Object.entries(currentStack.variables || {})
        ...
        .map(([name, value]) => variableRowHtml(name, value))
        .join("");
    showStackModal("Stack variables", "Static string variables available to macros and variables slots.", '<div class="modal-toolbar"><button id="addVariableBtn" data-icon="+" title="Add a static stack variable">Add variable</button><span class="modal-spacer"></span><span class="modal-meta">Save writes these changes to the stack JSON.</span></div>' +
        '<div class="data-table" id="variablesRows">' +
        '<div class="data-row header variable-row"><div>Name</div><div>Value</div><div></div></div>' +
        rows +
        '</div>');
...
function syncVariablesFromModal() {
    ...
    if (Object.keys(variables).length)
        currentStack.variables = variables;
    else
        delete currentStack.variables;
    stackVariablesError = duplicate ? "Duplicate stack variable names." : "";
    markDirty();
    if (duplicate)
        setStatus(stackVariablesError, "error");
}
...
function nextNumericItemId() {
    ...
    let index = 1;
    while (existing.has(String(index)))
        index++;
    return String(index);
}
function deleteSelectedItem() {
    if (!currentStack || selectedItemIndex < 0)
        return;
    const item = currentStack.items[selectedItemIndex];
    if (!confirm("Delete item " + item.id + "?"))
        return;
...
async function createAndOpenStack(stack, activate, actionLabel, extraOptions = {}) {
    ...
    if (data.importReport)
        showImportReport(data.importReport, selectedId);
}
async function createNewStack() {
    ...
    if (!id)
        throw new Error("Stack id must not be empty.");
    if (id !== promptedId.trim() && !confirm("Use stack id '" + id + "'?"))
        return;
...
function showImportReport(report, stackId) {
    showStackModal("SillyTavern import report", stackId || "", '<div class="modal-toolbar"><button id="copyImportReportBtn" data-icon="□" title="Copy this import report">Copy report</button><span class="modal-spacer"></span><span class="modal-meta">Report-only notes; stack changes are already saved.</span></div>' +
...
async function handleImportFile(event) {
    ...
    if (!Array.isArray(stack.items))
        throw new Error("Imported stack must contain an items array.");
    if (!stack.schemaVersion)
        stack.schemaVersion = 1;
    if (!stack.type)
        stack.type = "pi-forge.prompt-stack";
...
function stackForSubmit() {
    if (!currentStack)
        throw new Error("No stack selected.");
    if (optionsError)
        throw new Error("Invalid item options JSON: " + optionsError);
    if (stackVariablesError)
        throw new Error(stackVariablesError);
    if (regexEditor.getError())
        throw new Error(regexEditor.getError());
    if (policyEditor.getError())
        throw new Error(policyEditor.getError());
    const clone = structuredClone(currentStack);
    if (!clone.type)
        clone.type = "pi-forge.prompt-stack";
    if (!clone.schemaVersion)
...
function displayItemName(item) {
    if (item.name)
        return item.name;
    if (item.source && typeof item.source.previousName === "string" && item.source.previousName.trim())
        return item.source.previousName;
    if (item.kind === "slot" && item.slot)
        return item.slot;
...
function handleEditorShortcut(event) {
    if (event.key === "Escape") {
        if (el("preview").classList.contains("open"))
            hidePreview();
        else if (el("stackModal").classList.contains("open"))
            closeStackModal();
        return;
    }
