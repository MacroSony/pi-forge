...
- The no-egress `forge_subagent_profiles` tool gives the main agent a live catalog of loaded profile IDs, names, descriptions, model/thinking/stack metadata, and ready/unavailable resolution status. It also reports whether the parent tool policy currently permits `forge_subagent`.
...
- Backend selection is layered configuration rather than profile schema: `subagents.backend` in `~/.pi/forge/config.json` sets the user-level default, a trusted project's `.pi/forge/config.json` overrides it, and `/forge-agent plan|run --backend <id>` or the interactive `forge_subagent` `backend` parameter overrides one run. Both the `pi-subprocess-readonly` and `pi-rpc-readonly` backends are registered; there is no fallback to another backend when the selected one is unavailable, and unattended tool invocation is pinned to the configured default. `forge_subagent_profiles` and `/forge-agent backends` report the resolved default and its source.
- Foreground timeout is layered host configuration rather than profile schema: `subagents.timeoutMs` accepts 1,000 through 3,600,000 milliseconds, defaults to 60,000, and follows user-level then trusted-project precedence. Invalid values warn and preserve the previous valid/default value; the sealed request and all discovery, planning, and approval surfaces show the effective best-effort timeout.
