...
- Minimum repositories and codecs (Lane 2a): prompt-stack and agent-profile writes/deletes go through scoped repositories (`src/repositories/`) with containment and symlink checks, and all domain-resource serialization uses the single codec serializers in `src/codecs/`; the web host and profile service no longer touch the filesystem directly for these resources. Replacement semantics are unchanged and characterized by tests (no fingerprint conflict or atomic replace yet).
