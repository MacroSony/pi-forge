...
- **Configurable foreground subagent timeout.** `subagents.timeoutMs` now configures the best-effort timeout from 1,000 through 3,600,000 milliseconds with a 60,000-millisecond default. User-level configuration applies globally, trusted-project configuration overrides it, malformed values fail back to the previous valid/default value with a warning, and discovery, backend, plan, and approval surfaces show the effective timeout.
- **Published subagent runtime integration.** Pinned `@zihanw/pi-subagent-runtime@0.1.0-beta.1` instead of a sibling `file:` dependency. Host compilation now consumes the runtime's validated accepted preflight directly, and preparation forwards its `AbortSignal` through the public runtime API, removing the temporary `hostCompilePreflight()` and `prepareWithAbort()` shims.
