...
- **Agent-profile browser foundation.** The localhost editor now has top-level navigation between prompt stacks and project agent profiles. The first read-only profile slice lists exact model/thinking/stack targets, auto-activation and last-applied markers, current-to-target transitions, effective tools, shared resolution diagnostics, applicability, provenance, and runtime drift through a token-protected API backed by the existing profile services.
