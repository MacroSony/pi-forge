...
- Minimum repositories and codecs (Lane 2a): prompt-stack and agent-profile parse/normalize/validate/serialize are centralized in `src/codecs/`, and writes/deletes go through scoped repositories (`src/repositories/`) with containment and symlink checks; the web host and profile service no longer touch the filesystem directly for these resources, and the loaders delegate parsing to the codecs. Replacement semantics are unchanged and characterized by tests (no fingerprint conflict or atomic replace yet).
- ForgeWorkspace and host port v1 (Lane 2b): a minimal `ForgeWorkspace` snapshot owner over the repositories/codecs, plus a versioned `@zihanw/pi-forge/subagent` host-port protocol over the Pi event bus with the three minimal operations (discovery, profile listing/snapshot, prepare) and mandatory lifecycle semantics (bounded timeouts, duplicate-host failure, generation, disposal/unavailable, and listener cleanup), covered by tests.
