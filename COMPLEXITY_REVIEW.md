...
pi-forge is still a reasonable size for its feature set. The main engineering risk has moved from a few monolithic files to the safety and lifecycle seams of the experimental subagent backend. The next iteration should harden those seams while adding only the narrow parent-agent integration described in `NEXT_STEPS.md`.
...
|---|---:|---:|
| Production source (`src`) | 66 | 14,761 |
| Core tests and helpers (`tests`) | 20 | 6,650 |
...
- The full verification baseline passes: 175 core tests, one browser test, TypeScript typecheck, generated-client and `dist` consistency, and npm package-layout checks.
...
`src/index.ts` is now 194 lines and primarily wires prompt-stack, profile, tool-policy, web-editor, lifecycle, and subagent runtimes. Operational behavior lives in dedicated modules under `src/runtime/`.
...
| `src/subagent/backend-registry.ts` | 556 | Highest semantic risk: dispatch, cancellation, timeout, binding, and trace routing |
...
The registry and Pi SDK backend are not too large solely by line count, but they carry the strongest correctness and egress guarantees. Prefer focused helpers and end-to-end lifecycle tests over broad rewrites.

## Near-Term Priorities

1. Keep provider egress fail-closed in interactive, non-UI, cancelled, and timed-out paths.
2. Add the smallest model-callable parent tool without creating a second runner or allowing a tool call to grant its own egress consent.
3. Preserve `access: none`, foreground text execution, bounded result projection, and no automatic parent-history export for the first parent integration.
...
Avoid building retries, queues, chains, background execution, filesystem/process access, media routing, or artifact/trace storage into the 0.4 walking skeleton. Evaluate external orchestration packages behind the existing backend contract before expanding pi-forge into a general runner.

## Final Assessment

The original concentration problems have been addressed successfully. The codebase is ready for a narrow parent-agent integration after its backend safety tests remain green. Future complexity control should be incremental: isolate lifecycle-sensitive backend logic, split the browser client along real product boundaries, and resist turning pi-forge into a general orchestration framework without demonstrated need.
